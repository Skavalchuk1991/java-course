...
/**
 * Entry point of the Music Streaming Service application. All objects are created through the
 * hierarchy: MusicService is the root, everything flows down from it like a tree (artists with
 * albums, users with library/playlists/reviews/notifications).
 */
public class AppRunner {
  ...
  public static void main(String[] args) {
    ...
    for (Map.Entry<User, List<Media>> entry : history.entrySet()) {
      LOGGER.debug(
          "  User: {} -> {} tracks", entry.getKey().getUsername(), entry.getValue().size());
    }
    ...
    BiFunction<User, Media, String> listenMessage =
        (listeningUser, media) ->
    ...
    BiConsumer<User, Media> logListening =
        (listeningUser, media) ->
    ...
    MediaTransformer<Song, String> songFormatter =
        song -> song.getTitle() + " by " + song.getArtist() + " (" + song.getDuration() + "s)";
    LOGGER.info("Formatted: {}", songFormatter.transform(song1));

    // MediaAction<Media> — action on media
    MediaAction<Media> playAction =
        media -> LOGGER.info("Custom action: Playing {}", media.getTitle());
    ...
    LOGGER.info(
        "Missing media: {}",
        missing.orElse(new Song(0, "Fallback Song", 0, "Unknown", "Unknown", genre)));
    ...
    WordCounter.countUniqueWords(
        "src/main/resources/book.txt", "src/main/resources/unique_words.txt");
    ...
    java.util.concurrent.CompletableFuture<Void> future2 =
        java.util.concurrent.CompletableFuture.supplyAsync(() -> "sergey")
            .thenAccept(username -> LOGGER.info("Future2: Welcome back, {}!", username));
    future2.join();

    // 3. supplyAsync + thenCombine (combine two futures)
    java.util.concurrent.CompletableFuture<String> songFuture =
        java.util.concurrent.CompletableFuture.supplyAsync(() -> "Blinding Lights");
    java.util.concurrent.CompletableFuture<String> artistFuture =
        java.util.concurrent.CompletableFuture.supplyAsync(() -> "The Weeknd");
    java.util.concurrent.CompletableFuture<String> future3 =
        songFuture.thenCombine(artistFuture, (song, performer) -> song + " by " + performer);
    LOGGER.info("Future3 combined: {}", future3.join());

    // 4. runAsync (no input, no output — fire and forget)
    java.util.concurrent.CompletableFuture<Void> future4 =
        java.util.concurrent.CompletableFuture.runAsync(
            () -> LOGGER.info("Future4: Background cache cleanup executed"));
    future4.join();

    // 5. supplyAsync + exceptionally (error handling)
    java.util.concurrent.CompletableFuture<String> future5 =
        java.util.concurrent.CompletableFuture.supplyAsync(
                () -> {
    ...
    com.solvd.musicstreamingservice.xml.XsdValidator.validate(
        "src/main/resources/music-service.xml", "src/main/resources/music-service.xsd");

    // --- 2. StAX Parser ---
    LOGGER.info("\n--- 2. StAX Parser ---");
    com.solvd.musicstreamingservice.xml.Parser staxParser =
        new com.solvd.musicstreamingservice.xml.StaxParser();
    com.solvd.musicstreamingservice.xml.model.XmlMusicService staxResult =
        staxParser.parse("src/main/resources/music-service.xml");
    ...
    // --- 3. JAXB Parser ---
    LOGGER.info("\n--- 3. JAXB Parser ---");
    com.solvd.musicstreamingservice.xml.Parser jaxbParser =
        new com.solvd.musicstreamingservice.xml.JaxbParser();
    com.solvd.musicstreamingservice.xml.model.XmlMusicService jaxbResult =
        jaxbParser.parse("src/main/resources/music-service.xml");
    LOGGER.info("JAXB result: {}", jaxbResult);

    // --- 4. Jackson Parser (JSON) ---
    LOGGER.info("\n--- 4. Jackson Parser (JSON) ---");
    com.solvd.musicstreamingservice.xml.Parser jacksonParser =
        new com.solvd.musicstreamingservice.xml.JacksonParser();
    com.solvd.musicstreamingservice.xml.model.XmlMusicService jacksonResult =
        jacksonParser.parse("src/main/resources/music-service.json");
