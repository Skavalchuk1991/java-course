...
/**
 * StAX (Streaming API for XML) parser implementation. StAX is a pull parser — the application pulls
 * events from the parser on demand.
 *
 * <p>XPath examples for this XML structure:
 *
 * <ul>
 *   <li>{@code /musicService/serviceName} — select the service name
 *   <li>{@code /musicService/artists/artist/name} — select all artist names
 *   <li>{@code /musicService/artists/artist/albums/album/songs/song[@id='1']/title} — select song
 *       title with id=1
 *   <li>{@code /musicService/users/user[@id='1']/username} — select username of user with id=1
 *   <li>{@code //song[duration > 200]/title} — select titles of songs longer than 200 seconds
 * </ul>
 */
            ...
            if (inSongs && currentSong != null) {
              switch (currentElement) {
                ...
                case "availableOffline" ->
                    currentSong.setAvailableOffline(Boolean.parseBoolean(text));
