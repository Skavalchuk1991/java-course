...
import com.solvd.musicstreamingservice.exception.InvalidSubscriptionException;
import com.solvd.musicstreamingservice.exception.MediaLoadException;
import com.solvd.musicstreamingservice.exception.UserNotFoundException;
import com.solvd.musicstreamingservice.model.Artist;
import com.solvd.musicstreamingservice.model.Downloadable;
import com.solvd.musicstreamingservice.model.Genre;
import com.solvd.musicstreamingservice.model.Media;
import com.solvd.musicstreamingservice.model.Playable;
import com.solvd.musicstreamingservice.model.Reviewable;
...
public class MusicService {
  ...
  /** Business method: Streams media for a user */
  public void streamMedia(User user, Media media) {
    ...
    if (user.getSubscription() == null) {
      throw new InvalidSubscriptionException(
          "User '" + user.getUsername() + "' has no valid subscription");
    }
  ...
  /**
   * Simulates loading media from an external source. Throws checked MediaLoadException if media
   * cannot be loaded.
   */
  ...
  /**
   * Demonstrates polymorphism: works with any Media subtype. Calls overridden getMediaInfo() —
   * different result for Song vs Podcast.
   */
  ...
  /**
   * Polymorphism via interface: Works with any object that implements Playable — Song, Podcast,
   * etc.
   */
  ...
  /** Polymorphism via interface: Works with any object that implements Shareable. */
  ...
  /**
   * Polymorphism via Downloadable interface. Works with any class that explicitly implements
   * Downloadable.
   */
  ...
  /**
   * Polymorphism via Reviewable interface. Works with any class that explicitly implements
   * Reviewable.
   */
  ...
  public List<Media> filterCatalog(Predicate<Media> condition) {
    return catalog.stream().filter(condition).collect(Collectors.toList());
  }
  ...
  public <R> List<R> mapCatalog(Function<Media, R> mapper) {
    return catalog.stream().map(mapper).collect(Collectors.toList());
  }
  ...
  /** Stream: count media longer than given duration. Uses filter + count (terminal). */
  public long countMediaLongerThan(int seconds) {
    return catalog.stream().filter(media -> media.getDuration() > seconds).count();
  }

  /** Stream: get sorted catalog titles. Uses map + sorted + collect (non-terminal + terminal). */
  public List<String> getSortedTitles() {
    return catalog.stream().map(Media::getTitle).sorted().collect(Collectors.toList());
  }

  /** Stream: get total duration of all media. Uses mapToInt + sum (terminal). */
  public int getTotalCatalogDuration() {
    return catalog.stream().mapToInt(Media::getDuration).sum();
  }

  /**
   * Stream + Optional: find media by title. Returns Optional to handle case when media is not
   * found.
   */
  public Optional<Media> findMediaByTitle(String title) {
    return catalog.stream().filter(media -> media.getTitle().equalsIgnoreCase(title)).findFirst();
  }
